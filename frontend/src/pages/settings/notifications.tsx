    ...
    <div className="max-w-2xl mx-auto space-y-6 md:space-y-8 animate-in fade-in">
      ...
      <div className="space-y-5 md:space-y-6">
        {notifCategories.map((category) => (
          ...
          >
            <div className="bg-neutral-800/40 px-5 py-3.5 border-b border-neutral-800">
              <h3 className="font-semibold text-neutral-300 text-sm tracking-wide">
                {category.title}
              </h3>
            </div>
            <div className="flex flex-col">
              {category.items.map((notif, index) => (
                <div
                  key={notif.key}
                  className={`flex items-center justify-between p-5 ${index !== category.items.length - 1 ? "border-b border-neutral-800/50" : ""}`}
                >
                  <div className="pr-6">
                  ...
                  <button
                    ...
                    className={`w-12 h-7 flex items-center rounded-full px-1 transition-colors shrink-0 outline-none ${
                      settings[notif.key] ? "bg-[#0095f6]" : "bg-neutral-700"
                    }`}
                  >
                    <div
                      className={`w-5 h-5 bg-white rounded-full shadow-sm transition-transform duration-200 ${settings[notif.key] ? "translate-x-5" : "translate-x-0"}`}
                    />
                  </button>
                </div>
              ))}
